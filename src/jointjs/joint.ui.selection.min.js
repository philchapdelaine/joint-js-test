joint.ui.Selection = joint.mvc.View.extend({
    options: {
        ...
        boxContent: function (a) {
            return joint.util.template("<%= length %> elements selected.")({length: this.model.length})
        },
        handles: [{name: "remove", position: "nw", events: {pointerdown: "removeElements"}}, {
            name: "rotate",
            position: "sw",
    ...
    init: function () {
        ...
        joint.util.defaults(this.options, {graph: this.options.paper.model}), joint.util.bindAll(this, "startSelecting", "stopSelecting", "adjustSelection", "pointerup"), $(document.body).on("mousemove.selection touchmove.selection", this.adjustSelection), $(document).on("mouseup.selection touchend.selection", this.pointerup);
        var b = this.options.paper, c = this.options.graph;
    ...
    addHandle: function (a) {
        this.handles.push(a);
        var b = $("<div/>", {"class": "handle " + (a.position || "") + " " + (a.name || ""), "data-action": a.name});
    ...
    stopSelecting: function (a) {
        ...
        switch (this._action) {
            case"selecting":
                var d = this.$el.offset(), e = this.$el.width(), f = this.$el.height();
                b = c.pageToLocalPoint(d.left, d.top);
                var h = c.scale();
                e /= h.sx, f /= h.sy;
                var i = g.rect(b.x, b.y, e, f), j = this.getElementsInSelectedArea(i), k = this.options.filter;
    ...
    startSelecting: function (a) {
        ...
        if (null != a.offsetX && null != a.offsetY && $.contains(d, a.target)) b = a.offsetX, c = a.offsetY; else {
            var e = $(d).offset(), f = d.scrollLeft, g = d.scrollTop;
    ...
    changeHandle: function (a, b) {
        ...
        return c && (this.removeHandle(a), this.addHandle(joint.util.merge({name: a}, c, b))), this
    },
    ...
    adjustSelection: function (a) {
        ...
        switch (this._action) {
            case"selecting":
                ...
                var d = parseInt(this.$el.css("left"), 10), e = parseInt(this.$el.css("top"), 10);
                this.$el.css({
                ...
                break;
            case"translating":
                var f = this.options.paper.snapToGrid({x: a.clientX, y: a.clientY}), g = f.x, h = f.y;
                if (b = g - this._snappedClientX, c = h - this._snappedClientY, b || c) {
                    if (this.translateSelectedElements(b, c), this.boxesUpdated) this.model.length > 1 && this.updateSelectionBoxes(); else {
    ...
    translateSelectedElements: function (a, b) {
        ...
        this.model.each(function (d) {
            if (!c[d.id]) {
                ...
                d.translate(a, b, e), d.getEmbeddedCells({deep: !0}).forEach(function (a) {
                    c[a.id] = !0
                });
    ...
    getElementsInSelectedArea: function (a) {
        var b = this.options.paper, c = {strict: this.options.strictSelection};
    ...
    updateSelectionBoxes: function () {
        if (this._boxCount) {
            ...
            for (var a = this.$el.children(".selection-box"), b = 0, c = a.length; b < c; b++) {
                var d = a[b], e = $(d).remove().attr("data-model"), f = this.model.get(e);
    ...
    pointermove: function (a) {
        if (this._action) {
            var b = this.options.paper.snapToGrid({x: a.clientX, y: a.clientY}),
                c = this.options.paper.snapToGrid({x: this._clientX, y: this._clientY}), d = b.x - c.x, e = b.y - c.y;
    ...
    /**
     * Traverse through the tree and fill the nonGroupElements (DFS)
     * @param {Object} root
     * @param {Object[]} nonGroupElements
     */
    getNonGroupElements: function (root, nonGroupElements) {

        var that = this;

        // visit current root
        if (root && root.get('name') !== 'group') {
            nonGroupElements.push(root);
        }

        // depthFirst algorithm
        children = root.getEmbeddedCells();
        children.forEach(function (child) {
            that.getNonGroupElements(child, nonGroupElements);
        });
    },
    startRotating: function (a) {
        ...
        this.model.each(function (cell) {
            var nonGroupElements = [];
            that.getNonGroupElements(cell, nonGroupElements);
            nonGroupElements.forEach(ele => {
                cells.push(ele);
            });
        });
    ...
    startResizing: function (a) {
        var b = this.options.paper, c = this.options.graph, d = b.options.gridSize, e = this.model.toArray(),
            f = c.getBBox(e), g = joint.util.invoke(e, "getBBox"), h = g.reduce(function (a, b) {
                return b.width < a ? b.width : a
            }, 1 / 0), i = g.reduce(function (a, b) {
                return b.height < a ? b.height : a
            }, 1 / 0);
        this._resize = {
            cells: c.getSubgraph(e, {deep: true}),
            bbox: f,
    ...
    doResize: function (a, b, c) {
        ...
        var f = e.width;
        var g = e.height;
        var h = Math.max(f + b, d.minWidth);
        var i = Math.max(g + c, d.minHeight);
